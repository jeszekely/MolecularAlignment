...
//	Prints the 2D wavefunction to a file
...
void print_wvfxn(Array_2D <T> &A, Array_1D <U> &X, Array_1D <U> &Y, int tindex)
{
    ...
    for (ii = 0; ii < A.Nx; ii += 1)
    {
		for (jj = 0; jj < A.Ny; jj += 1)
		{
...
template <typename T, typename U>
void print_wvfxn(Array_2D <T> &A, Array_1D <U> &X, Array_1D <U> &Y, int tindex, int iint, int jint)
{
	ofstream outdata;
	int ii,jj;
    double re,im;
    ...
    for (ii = 0; ii < A.Nx; ii += iint)
    {
		for (jj = 0; jj < A.Ny; jj += jint)
		{
			re = real(A.get_elem(ii,jj));
			im = imag(A.get_elem(ii,jj));
			outdata << scientific << X.grid[ii] << "\t" << Y.grid[jj] << "\t" << re << "\t" << im << endl;
